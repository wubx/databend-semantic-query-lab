...
function validateManifest(manifest) {
  const errors = [];
  if (manifest?.api_version !== "semantic.databend.dev/v1alpha1")
    errors.push("unsupported api_version");
  if (manifest?.kind !== "SemanticManifest")
    errors.push("kind must be SemanticManifest");
  ...
  for (const entity of manifest.entities || []) {
    if (!entity.source?.table && !entity.source?.sql)
      errors.push(`entity ${entity.name} requires source.table or source.sql`);
    const entityMembers = [
      ...
      ...(entity.metrics || []).map((member) => ({
        ...member,
        kind: "metric",
      })),
      ...(entity.filters || []).map((member) => ({
        ...member,
        kind: "filter",
      })),
    ];
    ...
    members.set(
      entity.name,
      new Map(entityMembers.map((member) => [member.name, member])),
    );

    if (
      entity.keys?.primary &&
      !members.get(entity.name).has(entity.keys.primary)
    ) {
      errors.push(
        `entity ${entity.name} primary key ${entity.keys.primary} does not exist`,
      );
    }
    for (const metric of entity.metrics || []) {
      if (!metric.type || !metric.expr)
        errors.push(
  ...
  for (const relationship of manifest.relationships || []) {
    if (
      ...
      errors.push(
        `relationship ${relationship.name} references an unknown entity`,
      );
    }
    if (
  ...
  for (const query of manifest.verified_queries || []) {
    if (!query.id || queryIds.has(query.id))
      errors.push(`verified query id must be unique: ${query.id}`);
...
function validateCubeQueryMembers(query, members, errors, queryId) {
  if (!query)
    return errors.push(`verified query ${queryId} requires cube_query`);
  for (const name of query.measures || [])
    validateMember(name, "metric", members, errors, queryId);
  for (const name of query.dimensions || [])
    validateMember(name, "dimension", members, errors, queryId);
  for (const item of query.timeDimensions || [])
    validateMember(item.dimension, "time_dimension", members, errors, queryId);
  for (const filter of query.filters || [])
    validateMember(filter.member, null, members, errors, queryId);
  for (const segment of query.segments || [])
    validateMember(segment, "filter", members, errors, queryId);
  for (const name of Object.keys(query.order || {}))
    validateMember(name, null, members, errors, queryId);
}
...
function validateMember(qualifiedName, expectedKind, members, errors, queryId) {
  ...
  if (!member)
    return errors.push(
      `verified query ${queryId} references unknown member ${qualifiedName}`,
    );
  const compatibleKind =
    expectedKind === "dimension" && member.kind === "time_dimension";
...
function uniqueNames(items, label, errors) {
  ...
  for (const item of items) {
    if (!item?.name) errors.push(`${label} name is required`);
      ...
      errors.push(`duplicate ${label} name: ${item.name}`);
