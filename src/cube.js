function cubeUrl(path) {
  const base = (
    process.env.CUBE_API_URL || "http://127.0.0.1:4000/cubejs-api/v1"
  ).replace(/\/$/, "");
  return `${base}/${path}`;
}
...
async function cubeRequest(path, query) {
  ...
  const headers = {};
  if (process.env.CUBE_API_TOKEN)
    headers.Authorization = process.env.CUBE_API_TOKEN;
  const deadline =
    Date.now() + Number(process.env.CUBE_QUERY_TIMEOUT_MS || 120000);
  for (;;) {
    const remaining = deadline - Date.now();
    if (remaining <= 0)
      throw new Error("Cube query timed out while waiting for completion");
...
async function getCubeSql(query) {
  ...
  if (!Array.isArray(sql) || typeof sql[0] !== "string")
    throw new Error("Cube 没有返回可执行 SQL");
...
module.exports = { cubeHealth, cubeUrl, executeCube, getCubeSql };
