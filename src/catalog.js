const semanticQueries = {
  S1: {
    ...
    title: '订单总数',
    route: 'semantic',
    description: '通过 Cube 认证指标统计订单总数。',
    question: '订单总数是多少？',
  ...
  S2: {
    ...
    title: '按状态统计订单金额',
    route: 'semantic',
    description: '按订单状态聚合 Cube 认证的订单金额指标。',
  ...
  S3: {
    ...
    title: '每月订单金额趋势',
    route: 'semantic',
    description: '按月聚合 Cube 认证的订单金额指标。',
...
WHERE o_orderstatus = 'F'
  ...
  AND EXISTS (
    SELECT 1
    FROM tpch_100.lineitem l2
    WHERE l2.l_orderkey = l1.l_orderkey
      AND l2.l_suppkey <> l1.l_suppkey
  )
  AND NOT EXISTS (
    SELECT 1
    FROM tpch_100.lineitem l3
...
function integer(value, min, max) {
  ...
  if (!Number.isInteger(result) || result < min || result > max) {
    throw new Error(`整数参数必须在 ${min} 到 ${max} 之间`);
  }
  return result;
}
...
function date(value) {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(result)) {
    throw new Error('日期参数必须使用 YYYY-MM-DD 格式');
  }
  return result;
}
...
function getQuery(id) {
  return semanticQueries[id] || tpchQueries[id];
}
