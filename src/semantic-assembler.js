...
function assembleManifest(modelPath = DEFAULT_MODEL_PATH) {
  const root = path.dirname(modelPath);
  const model = readYaml(modelPath);
  ...
  const entities = entityIncludes.map(
    (file) => readYaml(path.join(root, file)).entity,
  );
  const relationships = readInclude(root, includes.relationships);
  const verifiedQueries = readInclude(root, includes.verified_queries);
  const policy = readInclude(root, includes.policy);
  return {
...
function readInclude(root, relativePath) {
  return relativePath ? readYaml(path.join(root, relativePath)) : {};
}
