...
async function loadExamples() {
  ...
  elements.examples.innerHTML = queries.map((query) => `<button class="example" data-question="${escapeHtml(query.question)}">${query.id} · ${escapeHtml(query.title)}</button>`).join('');
...
function renderPlan(plan) {
  ...
  ].map(([key, value]) => `<div class="interpretation-row"><span>${key}</span><strong>${escapeHtml(value)}</strong></div>`).join('');
  elements.cubeQuery.textContent = plan.cubeQuery ? JSON.stringify(plan.cubeQuery, null, 2) : '此查询使用认证的 TPC-H SQL 模板。';
  elements.sql.textContent = plan.sql || '尚未生成';
  elements.validation.textContent = plan.validation?.valid ? '安全检查通过' : '安全检查失败';
  elements.validation.style.color = plan.validation?.valid ? '#55dfc5' : '#ff8da3';
...
async function explain() {
  ...
  setBusy(true);
  try {
    const response = await api('/api/query/explain', { method: 'POST', body: JSON.stringify({ sql: currentPlan.sql }) });
    elements.explainResult.textContent = response.rows.map((row) => Object.values(row).join(' ')).join('\n');
...
async function api(url, options = {}) {
  ...
  if (!response.ok) throw new Error(body.error || body.message || body.errors?.join('; ') || `HTTP ${response.status}`);
  return body;
}
...
function format(value) { return value == null ? 'null' : typeof value === 'object' ? JSON.stringify(value) : String(value); }
